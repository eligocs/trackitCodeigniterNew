...
function FijModoEdit(t) {
    $(t).parent().find("#bAcep").show(), $(t).parent().find("#bCanc").show(), $(t).parent().find("#bEdit").hide(), 
    $(t).parent().find("#bElim").hide(), $(t).parents("tr").attr("id", "editing");
}
...
function rowAcep(t) {
    ...
    ModoEdicion(n) && (IterarCamposEdit(i, function(t) {
        ...
        var plan = t.find("input").parent('td').data('plan');
        var hotel = t.find("input").parent('td').data('hotel');
        var room = t.find("input").parent('td').data('room'); 
        var season = t.find("input").parent('td').parent().parent().data('season'); 
        allData.push({
            'price':n,
            'plan':plan,
            'hotel':hotel,
    ...
    if(allData){
        $.ajax({
            type: "POST",
            url: BASE_URL+"AjaxRequest/saveroomRates",
            data: {
                allData:allData
            }
        }).done(function(data) {
...
function rowEdit(t) {
    var n = $(t).parents("tr"), i = n.find("td");
    ModoEdicion(n) || (IterarCamposEdit(i, function(t) {
        var n = t.html(), i = '<div style="display: none;">' + n + "</div>", o = '<input class="form-control input-sm"  value="' + n + '">';
        t.html(i + o);
    }), FijModoEdit(t));
}
...
$.fn.SetEditable = function(t) {
    ...
    params = $.extend(n, t), this.find("thead tr").append('<th name="buttons"></th>'), 
