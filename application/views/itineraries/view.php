...
<div class="page-container itinerary-view">
    <div class="page-content-wrapper">
        <div class="page-content">
                    ...
                    <a class="btn btn-outline-secondary float-end mt-1 me-2" onclick="Print_iti();" href="javscript:void(0)" title="print">Print</a>
                        <?php if( ( $iti->iti_status == 0 || $iti->iti_status == 6 ) && is_admin_or_manager_or_sales() ){ ?>
                            ...
                            <a title='Edit Itinerary'
                                ...
                                class='btn btn-outline-info mt-1 me-2 float-end'><i class="fa-solid fa-pen-to-square"></i>
                                Edit
                            </a>
                        <?php } ?>
                    <?php 
                  ...
                  if( ($user_role == 99 || $user_role == 98 || $user_role == 96 ) && $iti->iti_status == 9 && isset( $paymentDetails[0] ) ){
                  	...
                  	if( !empty($tend_d) && $tend_d <= $td ){
                  		...
                  		if( $check_if_request_sent ){
                  			...
                  			$rev_btn = "<a title='Send review link to customer' target='_blank' href='javascript: void(0)' class='btn btn-outline-secondary float-end me-2 sendReview_req' data-cusid = '{$iti->customer_id}' ><i class='fa-solid fa-comment-sms'></i> Send review link</a>";
                  		}
            ...
            <div class="bg-white booking_items col-md-12 mb-4 p-2 rounded-4 shadow-sm">
                <?php 
                if(!empty(get_iti_tour_start_date($iti->iti_id))){

                    ?>
                    <span><strong class="badge_lightpurple_pill">Travel Date:</strong> <?= date("d-m-Y", strtotime(get_iti_tour_start_date($iti->iti_id))); ?></span>
                <?php }
                ?>
                <?php
                // print_r($meal_pane_type[0]);die;
                if(!empty($meal_pane_type[0])){
                $meal_plan = $meal_pane_type[0];

                if(!empty($meal_plan->meal_plan_type)){
                ?>
                    <span ><strong class="badge_lightpurple_pill">Meal Plan Type:</strong> <?= $meal_plan->meal_plan_type; ?> </span>
                <?php 
                }
                }
                if(!empty($iti->is_flight) && $iti->is_flight == 1){
                    $flight = $flight_details[0];
                ?>
                    <span ><strong class="badge_lightpurple_pill">Flight:</strong> <?= $flight->flight_name; ?> </span>
                <?php
                }
                if( isset( $train_details ) && !empty( $train_details ) && $iti->is_train == 1 ){
                    $train = $train_details[0];
                ?>
                        <span ><strong class="badge_lightpurple_pill">Train :</strong><?= $train->train_name; ?> </span>
                <?php
                }
                if($iti->cab_category == 7){
                    ?>
                    <span ><strong class="badge_lightpurple_pill">Bus:</strong> <?= get_car_name($iti->cab_category); ?></span>
                <?php 
                }
                if($iti->cab_category != 7){
                
                ?>
                    <span ><strong class="badge_lightpurple_pill">Car :</strong> <?= get_car_name($iti->cab_category); ?></span>
                <?php }
                ?>
            </div>
            ...
            <div class="">
                <div class="box blue">
                    <div class="portlet-body">
                        ...
                        echo !empty($pay_detail->iti_package_type) ? "<div class='bg-white mb-4 p-3 text-center rounded-4 shadow-sm'><h4 class='badge bg-yellow-soft fs-7 mb-3 text-center'>{$pay_detail->iti_package_type}</h4>" : "";
                        if( $pay_detail->iti_booking_status == 0 ){
                            echo '<h1 class="text-center green uppercase fs-4 ">Booked Itinerary</h1>';
                        }else if( $pay_detail->iti_booking_status == 1 ){
                            echo '<h1 class="text-center text-danger uppercase fs-4 ">Itinerary On Hold</h1>';
                        }else{
                            echo '<h1 class="text-center text-danger uppercase fs-4 ">Itinerary Rejected By Manager</h1>';
                            echo "<p class='text-center'><strong class='red'> Reason: </strong> {$pay_detail->approved_note}</p></div>";
                        } ?>
                        <div class="mt-element-step">
                            <div class="row step-background-thin justify-content-between">
            ...
            <div id="update_iti_hold_status">
                ...
                <div class="bg-white mb-4 p-3 rounded-4 shadow-sm">
                    <?php $aadhar_card_img = !empty( $pay_detail->client_aadhar_card ) ? $pay_detail->client_aadhar_card : ""; $payment_screenshot = !empty( $pay_detail->payment_screenshot ) ? $pay_detail->payment_screenshot : ""; ?>
                    <div class="row">
                        <div class="col-md-4">
               ...
               if( is_admin_or_manager() && $pay_detail->iti_booking_status == 1 ){
                ...
                <div class="custom_card" id="update_rates_section">
                    <form id="frmappOnholditi">
                        <div class="row frm_section">
                            <div class="spinner_load" style="display: none;">
                                <i class="fa fa-refresh fa-spin fa-3x fa-fw"></i>
                                <span class="sr-only">Loading...</span>
                            </div>
                            <div class='col-md-12'>
                                <p class="fs-6 mb-0 text-center text-primary uppercase">
                                    <strong>Please View And Update Itinerary: </strong> 
                                </p>
                            </div>
                            ...
                            <div class="form-group col-md-3 remove_required">
                                <label class="control-label"><strong>Client Aadhar Card:</strong></label>
                            ...
                            <div class="form-group col-md-3 remove_required">
                                <div class="form-group2">
                                    <label class="control-label"><strong>Payment Screenshot:</strong></label>
                            ...
                            <div class='form-group col-md-3'>
                                ...
                                <span class="red" style="font-size: 12px;"> Note: This comment is also visible for agent.</span>
                            </div>
                            <div class="form-group col-md-3">
                                <label class="control-label"><strong>Package Type*:</strong></label>
                        ...
                        <div class="button_sec mt-3">
                            <button class="btn btn-success button-submit" type="submit" value="approved">
                            ...
                            <button type="button" class="btn red reject_iti_btn" data-toggle="modal"
                                data-target="#reject_iti_ModalS"><i class="fa-solid fa-xmark"></i> Reject
                            </button>
                        </div>
            ...
            <div class="mb-4 text-center">
                <h2 class="fs-5 text-danger">Rejected Itinerary</h2>
                <p class="my-0"> <span>Reason :</span> <strong><?php echo $iti->iti_reject_comment; ?></strong></p>
            </div>
            <?php } ?>
            <?php
            //$string = "Ø";
         ...
         if( ( (empty($get_rate_meta) || get_iti_booking_status($iti->iti_id)  == 3) || $iti->pending_price == 1 ) && ( ( ( ( is_manager() && !is_leads_manager() ) || is_admin() ) && ( $iti->pending_price == 1 || $iti->pending_price == 5 ) ) || ( is_teamleader() && $iti->pending_price == 5 ) ) ){ ?>
            <div class="bg-white p-3 rounded-4 shadow-sm mb-4" id="update_rates_section">
                <form id="submitRates">
                    <div class="row">
                    ...
                    <div class='standard form-group col-xl-2 col-md-3 my-2'>
                        <label class="control-label">
                            <strong><?= totalHotelCategory()[0]->hotel_category_name ?>:</strong>
                        </label>
                        <input required name="rate_meta[standard_rates]" type="number" class='form-control'>
                    </div>

                    <div class='deluxe form-group col-xl-2 col-md-3 my-2'>
                        <label class="control-label"><strong>
                            <?= totalHotelCategory()[1]->hotel_category_name ?>:</strong>
                        </label>
                        <input required name="rate_meta[deluxe_rates]" type="number" class='form-control'>
                    </div>

                    <div class='super_deluxe form-group col-xl-2 col-md-3 my-2'>
                        <label class="control-label">
                            <strong><?= totalHotelCategory()[2]->hotel_category_name ?>:</strong>
                        </label>
                        <input required name="rate_meta[super_deluxe_rates]" type="number" class='form-control'>
                    </div>

                    <div class='luxury form-group col-xl-2 col-md-3 my-2'>
                        <label class="control-label">
                            <strong><?= totalHotelCategory()[3]->hotel_category_name ?>:</strong>
                        </label>
                        <input required name="rate_meta[luxury_rates]" type="number" class='form-control'>
                    </div>
                    
                    <div class='form-group col-xl-2 col-md-3 my-2'>
                        <label class="control-label d-block"><strong>Below Base Price.:</strong></label>
                        <input type="checkbox" value="1" title="Check if price is below Base Price" class='form-check-input' id="below_bp">
                        <input name="per_person_ratemeta[below_base_price]" type="hidden" value="" class='form-control below_bp'>
                    </div>
                    <input name="per_person_ratemeta[inc_gst]" type="hidden" value="1" class='form-control incgst'>

                    <div class='form-group col-xl-2 col-md-3 my-2'>
                        <label class="control-label d-block text-nowrap"><strong>Add Per/Person Rate:</strong></label>
                    ...
                    <div class="perperson_section" style="display: none;">
                        <div class="row">
                            <div class='standard  form-group col-xl-3 col-md-4 my-2'>
                                <label class="control-label">
                                    <strong><?= totalHotelCategory()[0]->hotel_category_name ?> (Per/Person):</strong>
                                </label>
                                <input name="per_person_ratemeta[standard_rates]" type="number" class='form-control' placeholder="Deluxe Per/Person Cost">
                            </div>
                            <div class='deluxe form-group col-xl-3 col-md-4 my-2'>
                                <label class="control-label">
                                    <strong><?= totalHotelCategory()[1]->hotel_category_name ?> (Per/Person):</strong>
                                </label>
                                <input name="per_person_ratemeta[deluxe_rates]" type="number" class='form-control'
                                    placeholder="Super Deluxe Per/Person Cost">
                            </div>
                            <div class='super_deluxe form-group col-xl-3 col-md-4 my-2'>
                                <label class="control-label">
                                    <strong><?= totalHotelCategory()[2]->hotel_category_name ?> (Per/Person):</strong>
                                </label>
                                <input name="per_person_ratemeta[super_deluxe_rates]" type="number" class='form-control'
                                    placeholder="Luxury Per/Person Cost">
                            </div>
                            <div class='luxury form-group col-xl-3 col-md-4 my-2'>
                                <label class="control-label">
                                    <strong><?= totalHotelCategory()[3]->hotel_category_name ?> (Per/Person):</strong>
                                </label>
                            ...
                            <!--child rate-->
                            <div class='standard  form-group col-xl-3 col-md-4 my-2'>
                                <label class="control-label"><strong class="red"><?= totalHotelCategory()[0]->hotel_category_name ?> (Per/child):</strong><span style="font-size:10px; color: red;"> ( Leave empty if not exists)</span></label>
                                <input name="per_person_ratemeta[child_standard_rates]" type="number" class='form-control' placeholder="Deluxe Per/child Cost">
                            </div>
                            <div class='deluxe form-group col-xl-3 col-md-4 my-2'>
                                <label class="control-label"><strong class="red"><?= totalHotelCategory()[1]->hotel_category_name ?>  (Per/child):</strong><span style="font-size:10px; color: red;"> ( Leave empty if not exists)</span></label>
                                <input name="per_person_ratemeta[child_deluxe_rates]" type="number" class='form-control' placeholder="Super Deluxe Per/child Cost">
                            </div>
                            <div class='super_deluxe form-group col-xl-3 col-md-4 my-2'>
                                <label class="control-label"><strong class="red"><?= totalHotelCategory()[2]->hotel_category_name ?> (Per/child):</strong><span
                            ...
                            <div class='luxury form-group col-xl-3 col-md-4 my-2'>
                                <label class="control-label"><strong class="red"><?= totalHotelCategory()[3]->hotel_category_name ?> (Per/child):</strong><span
                                        style="font-size:10px; color: red;"> ( Leave empty if not exists)</span></label>
                    ...
                    <!--end perperson rate meta -->
                    <div class='luxury form-group col-md-4 my-2'>
                        <label class="control-label"><strong>Rate Comments*</strong>
                            <span class="red" style="font-size: 12px;"> Note: This comment is also visible for client.</span>
                        </label>
            ...
            <form id="frmDiscountSupermanager">
                ...
                <div class='luxury form-group col-md-4'>
                    <label class="control-label"><strong>Rate Comments*</strong><span class="red" style="font-size: 12px;"> Note: This comment is
            ...
            <div>
                        ...
                        <div class="table-responsive booking_details_pro" style="">
                            <table class="table table-bordered bg_white">
                                ...
                                foreach( $hotel_bookings as $hotel_book ){
                                    ...
                                    $view_btn = "<a title='View' href=" . site_url("hotelbooking/view/{$hotel_book->id}/{$hotel_book->iti_id}") . " class='btn_eye' target='_blank' ><i class='fa-solid fa-eye' aria-hidden='true'></i></a>";
                                ...
                                foreach( $cab_bookings as $c_book ){
                                    ...
                                    $view_btn = "<a title='View' href=" . site_url("vehiclesbooking/viewbooking/{$c_book->id}") . " class='btn_eye' target='_blank' ><i class='fa-solid fa-eye' aria-hidden='true'></i></a>";
                                ...
                                foreach( $vtf_bookings as $vtf_book ){
                                    ...
                                    $view_btn = "<a title='View' href=" . site_url("vehiclesbooking/viewvehiclebooking/{$c_book->id}/{$vtf_book->id}/{$vtf_book->iti_id}") . " class='btn_eye' target='_blank' ><i class='fa-solid fa-eye' aria-hidden='true'></i></a>";
                ...
                <table class="table table-bordered ">
                    ...
                    <div class="portlet box blue box mt-5">
                        <div class="portlet-title">
                ...
                <div class="form-group col-md-12 margin-top-30">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form id="reqPriceForm">
                                            ...
                                            <div class="checkbox">
                                                ...
                                                <label><input name="hotel_cat_dis[]" required type="checkbox"
                                                        value="Standard"> <span class="cr"><i
                                                            class="cr-icon glyphicon glyphicon-ok"></i></span><strong><?= totalHotelCategory()[0]->hotel_category_name ?></strong></label><br>
                                                <label><input name="hotel_cat_dis[]" required type="checkbox"
                                                        value="Deluxe"> <span class="cr"><i
                                                            class="cr-icon glyphicon glyphicon-ok"></i></span><strong><?= totalHotelCategory()[1]->hotel_category_name ?>
                                                        </strong></label><br>
                                                <label><input name="hotel_cat_dis[]" required type="checkbox"
                                                        value="Super Deluxe"> <span class="cr"><i
                                                            class="cr-icon glyphicon glyphicon-ok"></i></span><strong><?= totalHotelCategory()[2]->hotel_category_name ?></strong></label><br>
                                                <label><input type="checkbox" required name="hotel_cat_dis[]"
                                                        value="Luxury"> <span class="cr"><i
                                                            class="cr-icon glyphicon glyphicon-ok"></i></span><strong>5
                                                        Star</strong></label>
                                            </div>
                                        </div>
                                        <?php
                                        ...
                                        ?>
                                        <input type="hidden" class='' required name="send_request_to"
                                                        value='1'>
                    ...
                    <?php echo "<div class='badge bg-yellow-gold fs-7 fw-normal p-2'>Itinerary Sent " . $iti_sent_counter . " Times.</div>"; ?>
