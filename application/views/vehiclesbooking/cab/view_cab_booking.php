		...
		<div class="page-content">
			...
			<div class="portlet box blue">
				<div class="portlet-title">
					...
					<a class="btn btn-outline-secondary float-end me-2" target="_blank" href="<?php echo iti_view_single_link($cab_booking->iti_id); ?>" title="View Itinerary"><i class="fa-solid fa-eye"></i> View Itinerary</a>
					
				</div>
			</div>
			<!-- SHOW CAB BOOKING STATUS -->
			<div class="bg-white rounded-4 p-3 shadow-sm mb-4 d-flex justify-content-between align-items-center">
				...
				<div class="bg-white p-3 rounded-4 shadow-sm mb-4">
								...
								<?php /*
									<div  class="mail-info well ">
										...
										if( $cab_booking->booking_status != 8 && $cab_booking->booking_status != 7  ){ 
											echo '<a id="cab_email_sent" href="#" class="btn green uppercase ">Send</a>';
										}
							...
							<div  class="mail-info">
								...
								if( $role == 99 || $role == 98 || $role == 97  ){
									...
									/*if(  $cab_booking->is_approved_by_gm == 0 ){
										echo '<a id="hotel_quotation_sent" href="#" data-id="' . $cab_booking->id . '" class="btn green uppercase ">Send Quotation To GM</a>';
									...
									if( $cab_booking->booking_status != 8 && $cab_booking->booking_status != 7 ){ 
										...
										echo '<a id="cab_email_sent" href="#" class="btn btn-success btn-sm me-3"><i class="fa-solid fa-paper-plane"></i> Send</a>';
									}
									
									if( $cabbookingemail == 0  && ( $cab_booking->booking_status != 8 || $cab_booking->booking_status != 7 ) ){ 
										echo "<div class='btn blue btn-outline sbold'>Cab booking email not send yet.</div>";
									}else{ 
										echo "<div class='badge bg-primary p-2'><i class='fa-solid fa-check'></i> Cab booking Sent " . $cabbookingemail . " Times.</div>";
									}
					...
					<div class='text-center'>
						...
						<?php }else if( $cab_booking->booking_status == 9 ){
							echo "<a  href='" . site_url("vehiclesbooking/update_cab_details/{$cab_booking->id}") . "' title='Update Cab Info' href='javascript:void(0)' class='btn btn-outline-secondary'>
							<i class='fa-solid fa-pen-to-square'></i> &nbsp; Update Cab Details</a>";
						} ?>	
					</div>	
				</div>
					...
					<div class="modal-body">
						...
						<?php //Get Hotel Information 
							...
							<div class="form-group my-3">
								<label class="control-label" for="email">Transporter Emails*:</label>
								<input required type="text" readonly value="<?php echo $tra_emails; ?>" name="trans_emails" class="form-control" id="email" placeholder="Enter customer email" >
							</div>
							<div class="form-group my-3">
							...
							<div class="my-3">
								<input type="hidden" name="id" value="<?php echo $cab_booking->id; ?>">
								<button type="submit" id="sentIti_btn" class="btn btn-success">
