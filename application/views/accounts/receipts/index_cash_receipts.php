						...
						if( isset($invoices_listing) && !empty( $invoices_listing ) ){
							...
							foreach($invoices_listing as $invoice) {
								...
								<tr data-id={$invoice->id}>
									...
									<td><a href=" . site_url("accounts/update_receipt/{$invoice->id}") . " class='btn btn_pencil ajax_edit_hotel_table' title='Update Invoice' ><i class='fa fa-pencil'></i></a>
									<a href=" . site_url("accounts/view_receipt/{$invoice->id}") . " class='btn btn_eye' title='view' ><i class='fa fa-eye'></i></a>
									{$client_v}
									<a href='javascript:void(0)' class='btn btn_trash ajax_delete_bank'><i class='fa fa-trash-o'></i></a></td>
									<td> {$agent}</td>
								</tr>";
								$i++; 
							}
