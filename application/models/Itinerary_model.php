		...
		foreach ($query->result() as $row){   
			foreach($row as $key=>$val){  
				...
				switch( $key ){
					...
					case ( $key == "rates_meta" || $key == "per_person_ratemeta" || $key == "client_comment_status" || $key == "client_comment" || $key == "followup_status" ):
						$this->db->set($key, "");
					break; 
					case ( $key == "quotation_sent_date" || $key == "approved_price_date" || $key == "pending_price_date" || $key == "iti_decline_approved_date" ):
						$this->db->set($key, "");
					break;
