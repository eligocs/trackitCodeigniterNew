	...
	public function savecustomer(){
		...
		if( $user['role'] == '99' || $user['role'] == '98' || is_teamleader() ){
			...
			if ($this->form_validation->run() == FALSE){
				...
				if (isset($_POST['inp']['customer_contact']) && !empty($_POST['inp']['customer_contact'])) {
					...
					if( $customer_id ){
						...
						**/
						...
						echo '<div class="alert alert-danger"><strong>Error!</strong>Please Try again later! </div>';
			...
			foreach ($list as $customer) {
				...
				if( $role == 99 || $role == 98 ){
					...
					if( $customer->cus_status == 8 ){
						...
						$reopen_s ="<a class='btn btn-success btn_table' href=" . site_url("customers/view/{$customer->customer_id}/{$customer->temp_key}?action=reopen") . " title='Reopen'><i class='fa fa-refresh' aria-hidden='true'></i> Reopen </a>";
					}	
			...
			if( !empty($list) ){
				foreach ($list as $customer) {
					...
					if( $customer->reopen_status == 9 ){
						...
						$reopen_s = '<a class="btn btn-success btn_table" href="#" data-customer_id="'. $customer->customer_id .'" data-temp_key = "'. $customer->temp_key .'" id="reopenLead" title="Reopen"><i class="fa fa-refresh" aria-hidden="true"></i> Reopen</a> 
						<div id="rr"></div> ';
					}
					// View 
