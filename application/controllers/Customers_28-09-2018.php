	...
	public function ajax_customers_list(){
		...
		if( !empty($list) ){
			foreach ($list as $customer) {
				...
				if( $role == '95' ){
					$row[] = "<a href=" . site_url("search?id=").$customer->customer_id . " title='View Customer' class='btn btn-success' ><i class='fa-solid fa-eye'></i></a>" . $decUserStatus;
				}else{
					// View 
					$row[] = "<a href=" . site_url("search?id=") . $customer->customer_id . " title='View Customer' class='btn btn-success' ><i class='fa-solid fa-eye'></i></a>". $row_edit . $row_delete . $add_iti . $reassign . $reopen_s; 
				}
