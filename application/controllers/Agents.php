...
class Agents extends CI_Controller {
	...
	public function index(){
		...
		if( $user['role'] == '99'){
			if( $user['role'] == '99'){
				if(!empty($_POST["filter"] || $_POST["userStatus"])){ 
					$where["user_type"] = $_POST['filter'];
					$where["userStatus"] = $_POST['userStatus'];
					$list = $this->login_model->get_datatables($where, $config["per_page"], $page);
				}else{
					$where = '';
					$list = $this->login_model->get_datatables($where, $config["per_page"], $page);
				}
				...
				$list = $this->login_model->get_datatables( $where_user_type , $config["per_page"], $page);
			...
			$this->load->view('users/agents', $data);
	...
	public function ajax_list(){
		$config = array();
        $config["base_url"] = base_url() . "agents/ajax_list";
        $config["total_rows"] = $this->login_model->get_count();
        $config["per_page"] = 4;
        $config["uri_segment"] = 3;
		$this->pagination->initialize($config);
		...
		if( $user['role'] == '99'){
			...
			// $this->load->view('inc/header');
			// $this->load->view('inc/sidebar');
			$this->load->view('ajaxTable/alluser', $data);
			// $this->load->view('inc/footer');
		}
	}
	...
	public function ajax_list_old(){
		...
		if( $user['role'] == '99'){
			if(!empty($_POST["filter"] || $_POST["userStatus"])){ 
				$where["user_type"] = $_POST['filter'];
				$where["userStatus"] = $_POST['userStatus'];
				$list = $this->login_model->get_datatables($where);
			}else{
		...
		if( !empty($list) ){
			foreach ($list as $agent) {
				if( $u_id !=  $agent->user_id && $agent->is_super_admin != "1" ){
					...
					$row[] = "<a title='view' href=" . site_url("agents/view/{$agent->user_id}") . " class='btn btn_eye' ><i class='fa-solid fa-eye'></i></a>" . $rowedit . $row1;
