...
class Itineraries extends CI_Controller {
	...
	public function view(){
		...
		if( !empty( $iti_id ) && !empty( $temp_key ) ){
			...
			if( $user['role'] == '99' || $user['role'] == '98' || $user['role'] == '97'){
				$where = array("del_status" => 0, "iti_id" => $iti_id, "temp_key" => $temp_key);
				$data['itinerary'] 	= $this->global_model->getdata( 'itinerary', $where );
				$iti = $data['itinerary'][0];
				$data['meal_pane_type'] = $this->global_model->getdata( 'customers_inquery', array( "customer_id" => $iti->customer_id ) );
	...
	public function ajax_itinerary_list(){
		...
		$custom_where = "";
		// if( $role == '99' || $role == '98' ){
		$list = $this->itinerary_model->get_datatables($custom_where );
		$data = array();
		$no = $_POST['start'];
		if( !empty($list) ){
			foreach ($list as $iti) {
		...
		<tbody>
			<tr>
					...
					<div class="align-bottom align-content-between d-flex flex-wrap h-100">
						<div class="d-flex justify-content-between px-1 w-100">
							<div class="requirment">
								<p title="Iti Id" class="fs-7 fw-bold mb-2 mt-0 d-inline-block">#<?= $iti->customer_id ?>
								</p>
								<span>/</span>
								<p title="Lead Id" class="fs-7 fw-bold mb-2 mt-0 d-inline-block">#<?= $iti->iti_id ?></p>
				...
				<td>
					<div class="align-bottom align-content-between d-flex flex-wrap h-100">
						<div class="mb-2 px-2">
							<p class="fw-bold m-0"><?= date("d-F-Y", strtotime(get_iti_tour_start_date($iti->iti_id))) ?>
							</p>
				...
				<td>
					<div class="align-bottom align-content-between d-flex flex-wrap h-100">
						...
						<div class="bg-light p-1 w-100">
							...
							<p class="fs-8 fw-400 m-0 text-dark"><?=  date("d-F-Y", strtotime( $iti->lead_created )); ?></p>
						</div>
					</div>
				</td>
				<td>
				...
				<td>
					<div class="dropdown">
						...
						<ul class="dropdown-menu" aria-labelledby="dropdownMenuLink" style="">
							<li>
								<a class="dropdown-item" target='_blank' href="<?= site_url("itineraries/edit/$iti_id/$key") ?>"><i class="fa-solid fa-pen-to-square"></i> Edit</a>
							</li>
							<li>
								<a class="dropdown-item" target='_blank' href="<?=site_url("itineraries/view_iti/$iti_id/$key") ?>"><i class="fa-solid fa-eye"></i> View</a>
							</li>
							<li>
								<a class="dropdown-item" target='_blank' href="<?=site_url("itineraries/pdf/$iti_id/$key") ?>"><i class="fa-solid fa-file-pdf"></i> PDF</a>
							</li>
							<li>
								<a class="dropdown-item" target='_blank' href="<?= site_url("itineraries/duplicate/$iti_id") ?>"><i class="fa-solid fa-clone"></i> Clone</a>
							</li>
							<li>
								<a class="dropdown-item delete_iti_permanent" data-id="<?= $iti_id ?>" href="#"><i class="fa-solid fa-trash-can"></i> Delete</a>
							</li>
						</ul>
					</div>
				</td>
...
<?php
	...
	public function ajax_itinerary_list1(){
		$user = $this->session->userdata('logged_in');
		$u_id = $user['user_id'];
		$role = $user['role'];
		$custom_where = "";
		...
		if( !empty($list) ){
			foreach ($list as $iti) {
				...
				if( $pub_status == "publish" || $pub_status == "price pending" ){
					...
					$row[] = $btn_edit . "
						<a data-id={$iti_id} title='Delete Itinerary Permanent' href='javascript:void(0)' class='btn_trash delete_iti_permanent'><i class='fa-solid fa-trash-can' aria-hidden='true'></i></a>";
				}	
				//$row[] = $iti->added;
				$row[] = $sent_status;
...
// 					$flight_other_details			= strip_tags($_POST['flight_other_details']);
...
// 						"in_itinerary"		=> 1,
// 						"flight_other_details"		=> $flight_other_details,
// 					);
...
// 					$train_other_details			= strip_tags($_POST['train_other_details']);
...
// 						"train_class"		=> $f_class,
// 						"in_itinerary"		=> 1,
// 						"train_other_details"		=> $train_other_details,
// 					);
		...
		if( isset($_POST["temp_key"]) && isset( $_POST['step'] ) ){
			...
			if( $step == 1 ){
				...
				if( $isflights == 1 ){
					...
					$trip_r			= strip_tags($_POST['trip_r']);
					$flight_other_details			= strip_tags($_POST['flight_other_details']);
					$return_flight_name			= strip_tags($_POST['return_flight_name']);
					...
					$f_data = array(
						...
						"in_itinerary"		=> 1,
						"flight_other_details"		=> $flight_other_details,
						"return_flight_name"		=> $return_flight_name,
					);
				...
				if( $isTrains == 1 ){
					...
					$trip_r			= strip_tags($_POST['t_trip_r']);
					$train_other_details			= strip_tags($_POST['train_other_details']);
					$return_train_name			= strip_tags($_POST['return_train_name']);
...
// 	}
		...
		if( !empty( $iti_id ) && !empty($temp_key) ){
			...
			$get_iti = $this->global_model->getdata( 'itinerary', $where_i );
			//get view folder
			$view_file = !empty( $get_iti ) && $get_iti[0]->iti_type == 2  ? "itineraries/pdf_new3" : "itineraries/pdf_new3";
			$data['discountPriceData'] 	= $this->global_model->getdata( 'itinerary_discount_price_data', array("iti_id" => $iti_id, "price_status" => 0) );
			$data['flight_details'] = $this->global_model->getdata( "flight_details", array("iti_id" => $iti_id) );
			$data['train_details'] = $this->global_model->getdata( "train_details", array("iti_id" => $iti_id) );
			if( $user['role'] == '99' || $user['role'] == '98' || $user['role'] == '97'){
