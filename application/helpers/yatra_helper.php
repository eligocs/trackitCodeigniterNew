...
*/
	...
	*/
	...
	function getAdminPhone(){
		$ci = & get_instance();
		$contact_no = $ci->global_model->getdata( "homepage_setting", "", "contact_no","",1 );
		if( $contact_no ){
			$result = $contact_no;
		}else{
			$result = "062300 70005";
	...
	function getaddress(){
		$ci = & get_instance();
		$address = $ci->global_model->getdata( "homepage_setting", "", "address","",1 );
		if( $address ){
			$result = $address;
		}else{
			$result = "1st floor Verma Niwas, below ICICI Bank, BCS, Shimla, Himachal Pradesh 171009";
		}	
		return $result;
	...
	*/
	...
	/* tour end date */
	function get_iti_tour_end_date( $iti_id ) {
	     $tour_date = '';
		if(!empty($iti_id) ){
			$ci = & get_instance();
			$daywisemetas  = $ci->global_model->getdata( "itinerary", array("iti_id" => trim($iti_id)), "daywise_meta" );
	...
	function get_first_date_iti($iti_id){
	    if( !empty($iti_id) ){
			$ci = & get_instance();
			$key = $ci->global_model->getdata( "daywise_meta", array("iti_id" => trim($iti_id)), "temp_key" );
			$res = base_url( "itineraries/view/$iti_id/$key" );
		}else{
			$res = false;
		}	
...
// 	function chnage_iti_package_name(){
// 	    $all_cats = get_iti_package_category();
// 	    if(!empty($all_cats)){
...
// 	}
			...
			if( $iti_id ){
				...
				if($get_iti_package_category){
					...
					foreach( $get_iti_package_category as $book_cat ){
						...
						$selected = "";
						$dp = 0;
				// 		print_r($book_cat->name);
						if( !empty( $disabled_d ) && strtolower( trim($book_cat->name) ) == strtolower("2 Star") ){
							$dp = $disabled_d;
							$disabled = "";
							$selected = !empty($selected_cat) && strtolower($selected_cat) == strtolower("2 Star") ? "selected='selected'" : "";
						}else if( !empty( $disabled_sd ) && strtolower( trim($book_cat->name) ) == strtolower("3 Star") ){
							$dp = $disabled_sd;
							$disabled = "";
							$selected = !empty($selected_cat) && strtolower($selected_cat) == strtolower("3 Star") ? "selected='selected'"  : "";
						}else if( !empty( $disabled_l ) && strtolower( trim($book_cat->name) ) == strtolower("4 Star") ){
							$dp = $disabled_l;
							$disabled = "";
							$selected = !empty($selected_cat) && strtolower($selected_cat) == strtolower("4 Star") ? "selected='selected'"  : "";
						}else if( !empty( $disabled_sl ) && strtolower( trim($book_cat->name) ) == strtolower("5 Star") ){
							$dp = $disabled_sl;
							$disabled = "";
							$selected = !empty($selected_cat) && strtolower($selected_cat) == strtolower("5 Star") ? "selected='selected'" : "";
						}
	...
	*/
	...
	/* get dicount  data */
	function discountparantIti($iti_id){
		if($iti_id){ 
			$ci =& get_instance();
			$ci->db->select('*'); 
